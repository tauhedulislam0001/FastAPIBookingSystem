...
async def bid_submit(id: int, request: Request, db: Annotated[Session, Depends(get_db)]):
    ...
    success_customer = request.query_params.get("success_customer")
    success_driver = request.query_params.get("success_driver")
    return templates.TemplateResponse("driver_bid.html", {"trips": trips_by_id,"request": request,"error": error, "success": success, "error_driver": error_driver, "success_customer": success_customer, "error_customer": error_customer, "success_driver": success_driver})
...
async def trips_get(id: int,request: Request,db:Annotated[Session, Depends(get_db)]):
    ...
    try:
        user = await decode_token(token, db)
        bids = db.query(models.Bids).join(models.Drivers).filter(models.Bids.trip_id == id).all()
